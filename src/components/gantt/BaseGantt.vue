...
import { has } from '@/utils/lodash.util'
...
watch([() => props.tasks, () => props.milestones], () => {
  parseData()
})
...
function initGanttTemplates() {
  ...
  gantt.templates.quick_info_class = (start, end, task) => {
    if (!isShowDetail(task)) return 'task-details__hide'
    return 'task-details'
  }
  gantt.templates.quick_info_content = (start, end, task) => {
    if (!isShowDetail(task)) return ''
  ...
  gantt.templates.progress_text = (start, end, task) => {
    ...
    const posTop = -(task.$index * SIZE.ROW_HEIGHT + 12)
...
function parseData() {
  try {
    ...
    const data = props.tasks?.reduce((pre, cur) => {
      if (!cur.loaded && cur.hasChildren) {
        pre.push({
          id: getPseudoTaskId(cur.id),
          parent: cur.id
        })
      }
    ...
    error('gantt load data error:', e)
  }
}

/**
 * Calculates height of filling the rest of gantt and render it
 */
...
function isShowDetail(task) {
  return !isSpecialTasks(task.id) && props.showTooltip
}
...
function handleColumnRender(task, column) {
  ...
  switch (column.type) {
    case COLUMN_TYPE.TEXT:
      return new GanttComponent.GanttNormalText({
        propsData: {
    ...
    case COLUMN_TYPE.STATE_SELECT: {
      const isObject = typeof task.status === 'object'
      ...
      if (column.component) {
        return new column.component({
          propsData: {
            data: task,
            change: (data) => {
              emitColumnUpdate(column.name, data)
            }
          }
        })
      }
...
/**
 * Emit updated column data
 * @param {string} name - Column name
 * @param {Object} data - Task
 */
function emitColumnUpdate(name, data) {
  emit('column-update', name, data)
}
...
function loadChild(task) {
  ...
  props
    ...
    .then((res) => {
      gantt.open(task.id)
      removeTask(getPseudoTaskId(task.id))
      updateTaskInfo({ ...task, loading: false, loaded: true })
      res?.forEach((c) => {
        c.end_date = formatDate(genEndDate(c, 'plus'))
        gantt.addTask(c, task.id)
      })
    })
    .catch((e) => {
      error('展开工作项 error: %o', e)
...
function getPseudoTaskId(id) {
  return `${PSEUDO_CHILD_ID}-${id}`
}
</script>
